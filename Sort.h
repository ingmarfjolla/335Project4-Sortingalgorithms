// Code from textbook
// Modified by: Ingmar Fjolla
//Project Description : 
//The next step is to modify the code of heapsort, quicksort and mergesort, 
//such that a Comparator us used. For example, the signature for quicksort will become: etc etc
//
...
////*********HEAPSORT****************HEAPSORT****************HEAPSORT****************HEAPSORT****************HEAPSORT********
...
//-***************MERGE SORT STUFF ***************MERGE SORT STUFF***************MERGE SORT STUFF// 
/**
            ...
            int leftPos, int rightPos, int rightEnd, Comparator less_than)
{
    ...
    while( leftPos <= leftEnd && rightPos <= rightEnd )
        // (a <= b is saying that b cannot be less than a: !(b < a))
...
//***************MERGE SORT STUFF *************MERGE SORT STUFF ************MERGE SORT STUFF***************MERGE SORT STUFF********




//***************Quick SORT STUFF *************quikc SORT STUFF ************quick SORT STUFF***************quick SORT STUFF********
...
//////
/**
 * Obtain the middle index and return the middle value
 */
template <typename Comparable, typename Comparator>
const Comparable & middle( vector<Comparable> & a, int left, int right, Comparator less_than)
{
  // Obtain the middle index in the array
  int center = ( left + right) / 2;


  // Place data at the middle index at the rightmost position
  swap( a[ center ], a[ right] );
  // Pivot is now in the last position, return pivot
  return a[ right ];
}
/**
 * Internal quicksort method that makes recursive calls.
 ...
 */
template <typename Comparable, typename Comparator>
void quicksort2Middle( vector<Comparable> & a, int left, int right, Comparator less_than )
{
   if( left + 10 <= right )
  {
    // Obtain pivot based on middle implementation
    const Comparable & pivot = middle( a, left, right, less_than );
    // The pivot is now located at the second to last index
        
    // Begin partitioning
    // i is the leftmost element
    // j is the element before the last element
    int i = left, j = right - 1;
    for( ; ; )
    {
      // Recall that first, last, and middle elements were not sorted, so no prefix operators to immediately change the value of i and j
      while( less_than(a[i], pivot )) {
        i++;
       }
      while( less_than( pivot, a[j] ) ){
    ...
    swap( a[ i ], a[ right] );  // Restore pivot (located in last position, differently from median3)
        
    quicksort2Middle( a, left, i - 1 , less_than);     // Sort small elements
    quicksort2Middle( a, i + 1, right, less_than );    // Sort large elements
  }
  else { // Do an insertion sort on the subarray
    insertionSort( a, left, right ,less_than);
  }
...
void QuickSort2(vector<Comparable> &a, Comparator less_than) {
  // This function will sort a list based on a pivot that is the literal, center-most index
  quicksort2Middle( a, 0, a.size() - 1, less_than);
}


/**
 * Obtain the first index and return the value
 */
template <typename Comparable, typename Comparator>
const Comparable & first( vector<Comparable> & a, int left, int right, Comparator less_than)
{
  // Obtain the first index in the array
  int pivot_position = left;

  // Basically swap the first and last element
  swap( a[ pivot_position ], a[right] );
  // Pivot was the data element in a[left], now it is at the right
  // Pivot is now in the last position, return pivot
  return a[ right];
}
 ...
 */
template <typename Comparable, typename Comparator>
void quicksort3First( vector<Comparable> & a, int left, int right, Comparator less_than )
{
   if( left + 10 <= right )
  {
    ...
    quicksort3First( a, left, i - 1 , less_than);     // Sort small elements
    quicksort3First( a, i + 1, right, less_than );    // Sort large elements
  }
  else { // Do an insertion sort on the subarray
    insertionSort( a, left, right ,less_than);
  }
...
void QuickSort3(vector<Comparable> &a, Comparator less_than) {
  // This function will sort a list based on a pivot that is the first index
  quicksort3First( a, 0, a.size() - 1, less_than);
}
